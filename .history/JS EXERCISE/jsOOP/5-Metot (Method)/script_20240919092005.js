...
class Dikdortgen implements shape {
...
//* Açıklama: shape(şekil) interface'i, bir şeklin sahip olması gereken alanHesapla ve cevreHesapla metotlarını tanımlar. Dikdortgen sınıfı bu interface'i uyguladığı için bu metotları gerçekleştirmek zorundadır.
*/

// Şekil nesnesi için bir şablon oluşturma (interface benzeri)
function Sekil() {
  this.alanHesapla = function() {
    // Alt sınıflarda bu metodu override etmeli
    console.error("Bu metot alt sınıflarda override edilmelidir.");
  };
...
function Dikdortgen(kenar1, kenar2) {
  // Sekil nesnesinin özelliklerini miras al
  Sekil.call(this);
...
// Kullanım
let dikdortgen = new Dikdortgen(5, 3);
console.log(dikdortgen.alanHesapla()); // Çıktı: 15
...
//! Mixins
//? Bir nesneye dışarıdan yeni özellikler eklemek için kullanılan bir tekniktir.

function hareketEdilebilirMixin(obj) {
  obj.hareketEt = function() {
    console.log("Hareket ediyor...");
  };
}

let araba = {};
hareketEdilebilirMixin(araba);
araba.hareketEt(); // Çıktı: Hareket ediyor...