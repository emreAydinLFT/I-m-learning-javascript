...
for (let i=1; i < sayilar7.length; i++) {
  if (sayilar7[i] > enBuyukSayi) {
    enBuyukSayi = sayilar7[i];
    }
  }
  console.log("En büyük sayı:", enBuyukSayi);

//! Math metodu ile en büyük sayıyı bulma
  const sayilar8 = [3, 7, 2, 9, 5];
  const enBuyukSayi2 = Math.max(...sayilar8);
  ...
  //! Diziler, döngüler, fonkisyonlar özeti
/*
  function fonksiyonAdi(parametre1, parametre2, ...) {
    // Fonksiyonun yapacağı işlemler burada yazılır
    // ...
    return sonuç; // Fonksiyonun döndürdüğü değer (isteğe bağlı)
  }
  *