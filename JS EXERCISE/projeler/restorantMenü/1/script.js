...
const categories = [...new Set(menu.map(item => item.category))]; //! ['soğuk', 'ara', 'sıcak']

categories.forEach(category => {
    const option = document.createElement('option'); // Yeni bir <option> elementi oluştur
    option.value = category; // Kategori değeri olarak kullan
    option.textContent = category; // Kategori metni olarak göster
    filter.appendChild(option); // Oluşturulan <option> elementini filtreye ekle
});
